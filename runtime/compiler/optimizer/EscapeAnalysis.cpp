...
/** AR07 - Fix for illegal creation of awrtbari nodes without 3 nodes.
 * Commoning up EA operations for value-types.*/
...
void TR_EscapeAnalysis::makeContiguousLocalAllocation(Candidate *candidate)
   ...
   if (candidate->isExplicitlyInitialized() && (originalAllocationOpCode == TR::newvalue))
      {
         /** AR07- Commoning up EA operations for value-types.*/
               ...
               for (j = candidate->_fields->size()-1; j >= 0; j--)
                  ...
                  if (field._offset == i &&
                     ...
                     if (stackFieldLoad->getDataType() == TR::Address)
                        ...
                        // if(strncmp("avrora/avrora/syntax/elf/ELFParser",comp()->getMethodBeingCompiled()->classNameChars(),34) == 0)
                        // {
                        //    printf("\n Num Children: %d BCI: %d",heapFieldStore->getNumChildren(),heapFieldStore->getByteCodeIndex());
                        //    printf("\n7:Write Barrier inside ELF class.");
                        // }
                        /** AR07 Debug End*/
                        if (comp()->useCompressedPointers())
                           {
